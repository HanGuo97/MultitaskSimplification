...
def attention_decoder(scope, decoder_inputs, initial_state, encoder_states, enc_padding_mask, cell, initial_state_attention=False, pointer_gen=True, use_coverage=False, prev_coverage=None):
    """
    Args:
    ...
    with variable_scope.variable_scope(scope.Attention):
        ...
        def attention(decoder_state, coverage=None):
            ...
            # Pass the decoder state through a linear layer (this is W_s
            # s_t + b_attn in the paper)
            # shape (batch_size, attention_vec_size)
            decoder_features = linear(
                decoder_state, attention_vec_size, True,
                kernel_name="query_kernel", bias_name="query_bias")
            ...
            if use_coverage and coverage is not None:  # non-first step of coverage
                ...
                e = math_ops.reduce_sum(
                    v * math_ops.tanh(encoder_features + decoder_features + coverage_features), [2, 3])
                ...
                e = math_ops.reduce_sum(
                    v * math_ops.tanh(encoder_features + decoder_features), [2, 3])  # calculate e

                # Calculate attention distribution
                attn_dist = masked_attention(e)

                if use_coverage:  # first step of training
                    coverage = tf.expand_dims(tf.expand_dims(
                        attn_dist, 2), 2)  # initialize coverage

            # Calculate the context vector from attn_dist and
            # encoder_states
            # shape (batch_size, attn_size).
            # print("attn_dist", attn_dist)
            # print("attn_dist reshaped", array_ops.reshape(attn_dist, [batch_size, -1, 1, 1]))
            # print("encoder_states", encoder_states)
            # print("attn_dist reshaped", math_ops.reduce_sum(array_ops.reshape(
            #    attn_dist, [batch_size, -1, 1, 1]) * encoder_states, [1, 2]))

            context_vector = math_ops.reduce_sum(array_ops.reshape(
                attn_dist, [batch_size, -1, 1, 1]) * encoder_states, [1, 2])
            context_vector = array_ops.reshape(
                context_vector, [-1, attn_size])
        ...
        if initial_state_attention:  # true in decode mode
            ...
            context_vector, _, coverage = attention(initial_state[-1].h, coverage)
        ...
        for i, inp in enumerate(decoder_inputs):
            ...
            with variable_scope.variable_scope(scope.Attention,
                    reuse=i > 0 or scope.Attention.reuse):
                x = linear([inp] + [context_vector], input_size, True,
                    kernel_name="input_kernel", bias_name="input_bias")

            # Run the decoder RNN cell. cell_output = decoder state
            with variable_scope.variable_scope("decoder_scope"):
                cell_output, states = cell(x, states)
                state = states[-1]

            # Run the attention mechanism.
            with tf.variable_scope(scope.Attention,
                    reuse=initial_state_attention or i > 0 or scope.Attention.reuse):
                if i == 0 and initial_state_attention:  # always true in decode mode
                # you need this because you've already run the initial
                # attention(...) call

                    context_vector, attn_dist, _ = attention(
                        cell_output, coverage)  # don't allow coverage to update
                else:
                    context_vector, attn_dist, coverage = attention(
                        cell_output, coverage)
            ...
            if pointer_gen:
                with variable_scope.variable_scope(scope.Pointer,
                        reuse=(i != 0) or scope.Pointer.reuse):
                    p_gen = linear(
                        [context_vector, state.c, state.h, x], 1, True,
                        kernel_name="pgen_kernel", bias_name="pgen_bias")  # a scalar
            ...
            with variable_scope.variable_scope(scope.Attention,
                ...
                output = linear([cell_output] + [context_vector],
                    ...
                    kernel_name="output_kernel", bias_name="output_bias")
        ...
        return outputs, states, attn_dists, p_gens, coverage


def linear(args, output_size, bias, bias_start=0.0,
           kernel_name="Matrix", bias_name="Bias"):
    ...
    # Now the computation.
    matrix = tf.get_variable(kernel_name, [total_arg_size, output_size])
    if len(args) == 1:
    ...
    bias_term = tf.get_variable(
        bias_name, [output_size], initializer=tf.constant_initializer(bias_start))
